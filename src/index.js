...
class MoneyBook extends Component {
    ...
    addBook(date,item,amount){
        ...
        this.setState({books:this.books.concat(book)})
    }
...
class MoneyEntry extends Component{
    ...
    onChangePayingIn(e){
        this.setState({payingIn:e.target.value=='on'})
    }
    onClickSubmit(){
        ...
        return(
            <div className="entry">
                <fieldset>
                    ...
                    <div>
                        日付:<input type="text" value={this.state.date} name="date" onChange={(e)=>this.onChangeValue(e)} placeholder="3/15"/>
                    </div>
                    <div>
                        項目:<input type="text" value={this.state.item} name="item" onChange={(e)=>this.onChangeValue(e)} placeholder="おこづかい"/>
                    </div>
                    <div>
                        金額:<input type="text" value={this.state.amount} name="amount"onChange={(e)=>this.onChangeValue(e)} placeholder="1000"/>
                    </div>
                    <div>
                        <input type="submit" value="追加" onClicke={()=>this.onClickSubmit()} />
                    </div>
                </fieldset>
            </div>
        )
